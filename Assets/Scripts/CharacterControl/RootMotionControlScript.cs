...
public class RootMotionControlScript : MonoBehaviour
{
    ...
    void OnAnimatorMove()
    {
        ...
        Vector3 positionDifference = newRootPosition - this.transform.position;
        Quaternion rotationDifference = Quaternion.Inverse(this.transform.rotation) * newRootRotation;

        Vector3 scaledPosition = Vector3.LerpUnclamped(this.transform.position, this.transform.position + positionDifference, rootMovementSpeed);
        Quaternion scaledRotation = Quaternion.LerpUnclamped(this.transform.rotation, this.transform.rotation * rotationDifference, rootTurnSpeed);
        // old way
